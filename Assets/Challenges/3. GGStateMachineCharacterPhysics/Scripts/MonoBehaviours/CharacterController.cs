...
namespace Challenges._3._GGStateMachineCharacterPhysics.Scripts.MonoBehaviours
{
    ...
    public class CharacterMovementConfig
    {
        ...
        [Tooltip("How quickly the character speeds up: u/s^2")]
        private float accelerationByTime;
        [SerializeField, Range(0, 10)]
        [Tooltip("Maximum speed: u/s")]
        private float maxSpeed;
        [SerializeField, Range(0.9f, 1)]
        [Tooltip("Multiplied with speed every frame ")]
        private float generalVelocityDamping;
        [SerializeField, Range(0.9f, 1)]
        [Tooltip("Multiplied with speed every frame only when there's input")]
        private float withInputVelocityDamping;
        [SerializeField, Range(0.9f, 1)]
        [Tooltip("Multiplied with speed every frame only when there's no input")]
        private float noInputVelocityDamping;
        [SerializeField, Range(0.9f, 1)]
        [Tooltip("Multiplied with speed every frame only when the character is above ground")]
    ...
    public class CharacterController : MonoBehaviour, IInputListener
    {
        ...
        {
            transform.position += new Vector3(xzPlaneMovementVector.x, 0,xzPlaneMovementVector.y)*characterMovementConfig.MAXSpeed;
        }
